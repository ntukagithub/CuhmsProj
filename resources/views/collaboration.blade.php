...
<div class="border border-gray-600 py-2 px-5 w-[120px] absolute ml-[240px] mt-[20px] rounded-lg text-center hover:bg-gray-600 hover:text-white  hover:font-bold hover:cursor-pointer duration-300 "> <a href="{{route('dashboard')}}"><i class="fa-solid fa-house pr-2 text-white hover:text-white "></i>Home</a></div>
  <div>
  <div class="text-white max-w-[600px] mx-auto font-black  text-1xl text-center pt-1 border border-gray-600 mt-1 rounded-t-lg">Graduates Network</div>
   <div class=" max-w-[600px] h-[78vh] mx-auto border border-gray-600 overflow-scroll overflow-x-hidden overflow-y-auto  mt-0  ">
   ...
   <div class="border border-gray-600 w-[350px]  p-4 mt-1.5 ml-3 rounded-xl text-gray-300"> {{$data->content}}</div>
   </div>
   @endforeach
   <!-- <div class="flex ml-7"> 
   <div class="bg-white w-[40px] h-[40px] mt-2 ml-2 rounded-full overflow-hidden"> <img src="https://daisyui.com/images/stock/photo-1534528741775-53994a69daeb.jpg" /></div>
   ...
   <div >
            <form  method="post" action="/send-data" class="max-w-[600px] h-[7vh] bg-gray-600 mx-auto mt-[-3px] rounded-b flex shadow-lg " >
            @csrf
            ...
            <button type="submit" wire:click="$refresh" class="border h-[6vh] border-gray-900 bg-gray-900 w-[54px] mt-1 hover:bg-gray-600 duration-300 overflow-hidden hover:border-gray-600 rounded-r" wire:click="$refresh" > <i class="fa-solid fa-paper-plane text-1xl  text-white  "></i> </button>
            </form>
   </div>
   </div>
